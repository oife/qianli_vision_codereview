...
bool TensorRTBackend::infer(const cv::Mat & input, cv::Mat & output, BackendCtx * ctx)
{
  ...
  auto prepare_input_start = std::chrono::steady_clock::now();
  if (!prepare_input(input, host_input)) return false;
  auto prepare_input_end = std::chrono::steady_clock::now();
  last_profile_.prepare_input_ms =
    tools::delta_time(prepare_input_end, prepare_input_start) * 1000.0;
  ...
  last_profile_.h2d_submit_ms = tools::delta_time(h2d_end, h2d_start) * 1000.0;
  ...
  last_profile_.enqueue_submit_ms = tools::delta_time(enqueue_end, enqueue_start) * 1000.0;
  ...
  last_profile_.d2h_submit_ms = tools::delta_time(d2h_end, d2h_start) * 1000.0;
  ...
  last_profile_.sync_ms = tools::delta_time(sync_end, sync_start) * 1000.0;

  auto output_convert_start = std::chrono::steady_clock::now();
  ...
  auto output_convert_end = std::chrono::steady_clock::now();
  last_profile_.output_convert_ms =
    tools::delta_time(output_convert_end, output_convert_start) * 1000.0;
  last_profile_.infer_ms = tools::delta_time(output_convert_end, infer_start) * 1000.0;
  return true;
}
  ...
  const std::string & model_path, const std::string & engine_path)
{
  ...
  if (dynamic) {
    ...
    if (opt_dims.nbDims >= 4) {
      opt_dims.d[1] = model_config_.input_type == CV_8UC1 ? 1 : 3;
...
bool TensorRTBackend::ensure_context_ready(TensorRTCtx & ctx)
{
  ...
  if (input_dims.nbDims >= 4) {
    input_dims.d[0] = 1;
    input_dims.d[1] = model_config_.input_type == CV_8UC1 ? 1 : 3;
  ...
  if (!ctx.context_->setTensorAddress(input_name_.c_str(), ctx.input_device_) ||
...
bool TensorRTBackend::prepare_input(const cv::Mat & input, std::vector<float> & host_input) const
{
  ...
  for (int c = 0; c < channels; ++c) {
    std::memcpy(host_input.data() + c * plane, split_channels[c].ptr<float>(), plane * sizeof(float));
  }
  return true;
}
