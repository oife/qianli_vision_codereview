...
/**
 * @brief 检查灯条的几何特征是否符合要求
 * @param lightbar 待检查的灯条对象
 * @return 是否符合几何要求（角度误差、长宽比、长度）
 */
...
/**
 * @brief 检查装甲板的几何特征是否符合要求
 * @param armor 待检查的装甲板对象
 * @return 是否符合几何要求（长宽比、侧边比、矩形误差）
 */
 ...
 * @param armor 待检查的装甲板对象
 * @return 类型与名称是否匹配（小装甲板不能是1号或基地，大装甲板必须是1号或基地）
 */
...
/**
 * @brief 在调试模式下显示检测结果（二值化图像、灯条、装甲板）
 * @param binary_img 二值化图像
 * @param bgr_img 原始BGR彩色图像
 * @param lightbars 检测到的灯条列表
...
/**
 * @brief 使用PCA方法修正灯条的顶部和底部角点位置
 * @param lightbar 待修正的灯条对象（输入输出参数）
 * @param gray_img 灰度图像
 * @note 利用PCA回归角点，参考自https://github.com/CSU-FYT-Vision/FYT2024_vision
 */
